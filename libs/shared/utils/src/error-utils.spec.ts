...
describe('ErrorUtils', () => {
  ...
  describe('getErrorMessages', () => {
    ...
    test('should handle simple throwable', () => {
      ...
      expect(ErrorUtils.getErrorMessages(new Error('msg'))).toEqual([
        'Error(msg)',
      ]);
      expect(ErrorUtils.getErrorMessages(new Error(''))).toEqual(['Error']);
    });
  ...
  describe('getErrorMessages with addClassName=false', () => {
    ...
    test('should handle simple throwable without class names', () => {
      expect(ErrorUtils.getErrorMessages(new Error(), false)).toEqual([
        'Error',
      ]);
      expect(ErrorUtils.getErrorMessages(new Error('msg'), false)).toEqual([
        'msg',
      ]);
      expect(ErrorUtils.getErrorMessages(new Error(''), false)).toEqual([
        'Error',
  ...
  describe('getErrorClassNames and getExceptionFullClassNames', () => {
    ...
    test('should return class names for exception chain', () => {
      ...
      expect(ErrorUtils.getErrorClassNames(parent)).toEqual([
        'TypeError',
        'Error',
      ]);
    });
  ...
  describe('getNotEmptyMessageOrErrorClassSimpleName', () => {
    test('should return message if not empty', () => {
      expect(
        ErrorUtils.getNotEmptyMessageOrErrorClassSimpleName(new Error('msg'))
      ).toBe('msg');
    });

    test('should return class name if message is null/empty', () => {
      expect(
        ErrorUtils.getNotEmptyMessageOrErrorClassSimpleName(new Error())
      ).toBe('Error');
      expect(
        ErrorUtils.getNotEmptyMessageOrErrorClassSimpleName(new Error(''))
      ).toBe('Error');
  ...
  describe('validation methods', () => {
    test('isValidArgument should throw BusinessRuleError on false', () => {
      expect(() => ErrorUtils.isValidArgument(false, 'Message')).toThrow(
        BusinessRuleError
      );
      expect(() => ErrorUtils.isValidArgument(false, 'Message')).toThrow(
        '[BAD_REQUEST_GENERAL] Message'
      );
    });

    test('isFoundResource should throw BusinessRuleError on false', () => {
      expect(() => ErrorUtils.isFoundResource(false, 'Message')).toThrow(
        BusinessRuleError
      );
      expect(() => ErrorUtils.isFoundResource(false, 'Message')).toThrow(
        '[NOT_FOUND_GENERAL] Message'
      );
    });

    test('isAllowed should throw BusinessRuleError on false', () => {
      expect(() => ErrorUtils.isAllowed(false, 'Message')).toThrow(
        BusinessRuleError
      );
      expect(() => ErrorUtils.isAllowed(false, 'Message')).toThrow(
        '[FORBIDDEN_GENERAL] Message'
      );
    });

    test('isConflict should throw BusinessRuleError on false', () => {
      expect(() => ErrorUtils.isConflict(false, 'Message')).toThrow(
        BusinessRuleError
      );
      expect(() => ErrorUtils.isConflict(false, 'Message')).toThrow(
        '[CONFLICT_GENERAL] Message'
      );
    });
  });
  ...
  describe('trimAndValidateId', () => {
    ...
    test('should throw BusinessRuleError for null/empty/whitespace ids', () => {
      expect(() => ErrorUtils.trimAndValidateId(null as any, 'testId')).toThrow(
        BusinessRuleError
      );
      expect(() => ErrorUtils.trimAndValidateId(null as any, 'testId')).toThrow(
        '[BAD_REQUEST_GENERAL] Expected not empty testId'
      );

      expect(() => ErrorUtils.trimAndValidateId('', 'testId')).toThrow(
        BusinessRuleError
      );
      expect(() => ErrorUtils.trimAndValidateId('', 'testId')).toThrow(
        '[BAD_REQUEST_GENERAL] Expected not empty testId'
    ...
    test('should throw BusinessRuleError for invalid id characters', () => {
      expect(() =>
        ErrorUtils.trimAndValidateId('!@id==Wrong', 'testId')
      ).toThrow(BusinessRuleError);
      expect(() =>
        ErrorUtils.trimAndValidateId('!@id==Wrong', 'testId')
      ).toThrow('[BAD_REQUEST_GENERAL] Invalid testId: !@id==Wrong');
    });
    ...
    test('should throw BusinessRuleError for too long id', () => {
      const longId =
        'Very-Long-Long-ID-Very-Long-Long-ID-Very-Long-Long-ID-Very-Long-Long-ID-Very-Long-Long-ID-' +
        'x'.repeat(100);
      expect(() => ErrorUtils.trimAndValidateId(longId, 'testId')).toThrow(
        BusinessRuleError
      );
      expect(() => ErrorUtils.trimAndValidateId(longId, 'testId')).toThrow(
        '[BAD_REQUEST_GENERAL] Invalid testId:'
      );
    });

    test('should throw Error for invalid idName parameter', () => {
      expect(() =>
        ErrorUtils.trimAndValidateId('!@id==Wrong', null as any)
      ).toThrow('Expected not empty idName');
      expect(() => ErrorUtils.trimAndValidateId('!@id==Wrong', '')).toThrow(
        'Expected not empty idName'
      );
      expect(() =>
        ErrorUtils.trimAndValidateId('!@id==Wrong', '\r\n\t ')
      ).toThrow('Expected not empty idName');
    });

    test('should throw Error for invalid maxIdSize parameter', () => {
      expect(() =>
        ErrorUtils.trimAndValidateId('!@id==Wrong', 'idName', -1)
      ).toThrow('Expected maxIdSize to be positive: -1');
      expect(() =>
        ErrorUtils.trimAndValidateId('!@id==Wrong', 'idName', 0)
      ).toThrow('Expected maxIdSize to be positive: 0');
    });
  });
  ...
  describe('toBusinessRuleError', () => {
    test('should handle null ErrorDto', () => {
      ...
      expect(result.message).toBe(
        '[INTERNAL_SERVER_ERROR_GENERAL] Unknown error'
      );
    });
    ...
    test('should handle null ErrorDto with message', () => {
      ...
      expect(result).toBeInstanceOf(BusinessRuleError);
      expect(result.message).toBe(
        '[INTERNAL_SERVER_ERROR_GENERAL] Error message'
      );
    });
    ...
    test('should handle JSON error messages', () => {
      const result = ErrorUtils.toBusinessRuleError(
        401,
        '{"error": "Error message custom"}'
      );
      expect(result).toBeInstanceOf(BusinessRuleError);
      expect(result.message).toBe(
        '[UNAUTHORIZED_GENERAL] Error message custom'
      );
    });
    ...
    test('should handle JSON with both error and message fields', () => {
      const result = ErrorUtils.toBusinessRuleError(
        404,
        '{"error": "Error", "message": "Message custom"}'
      );
      expect(result).toBeInstanceOf(BusinessRuleError);
    ...
    test('should handle JSON with unknown fields', () => {
      const result = ErrorUtils.toBusinessRuleError(
        404,
        '{"custom": "Custom", "message": "Message custom"}'
      );
      expect(result).toBeInstanceOf(BusinessRuleError);
      expect(result.message).toContain(
        '[NOT_FOUND_GENERAL] {"custom": "Custom", "message": "Message custom"}'
      );
    });
    ...
    test('should handle formatting in error message', () => {
      ...
      expect(result).toBeInstanceOf(BusinessRuleError);
      expect(result.message).toBe(
        '[INTERNAL_SERVER_ERROR_GENERAL] Error message %s'
      );
    });
  });
  ...
  describe('sprintf formatting', () => {
    test('should format various placeholders correctly', () => {
      expect(() =>
        ...
        '[BAD_REQUEST_GENERAL] User John Doe with ID 12345 has balance $1234.57 and data: {"status":"active"}'
      );
    });
    ...
    test('BusinessRuleError constructor should support sprintf formatting', () => {
      ...
      expect(error.message).toBe(
        '[NOT_FOUND_GENERAL] Resource User with ID 42 not found'
      );
